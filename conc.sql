...
select fcq.zd_edition_name              edition
,      fcq.concurrent_queue_name        queue
,      fcq.concurrent_queue_id          queue_id
...
,      CASE
         ...
         ELSE
           TO_CHAR(extract(hour   from numtodsinterval(((fcr.actual_completion_date - fcr.actual_start_date)*86400),'SECOND'))+
                   24*extract(day from numtodsinterval(((fcr.actual_completion_date - fcr.actual_start_date)*86400),'SECOND')),'FM009')||'h'||
...
--from   dto.xxfnd_hist_concurrent_requests    fcr
,      apps.fnd_user                   fu1
